...
class RNN_VAE(nn.Module):
    ...
    def __init__(self, n_vocab, h_dim, z_dim, c_dim, p_word_dropout=0.3, unk_idx=0, pad_idx=1, start_idx=2, eos_idx=3, max_sent_len=15, pretrained_embeddings=None, freeze_embeddings=False, gpu=False, gpu_id=0):
    ...
    def sample_z(self, mu, logvar, EPS=True):
        """
        Reparameterization trick: z = mu + std*eps; eps ~ N(0, I)
        """
        eps = Variable(torch.randn(self.z_dim))
        eps = eps.cuda(self.gpu_id) if self.gpu else eps
        if EPS:
            return mu + torch.exp(logvar/2) * eps
        else:
            return mu + torch.exp(logvar/2)
    ...
    def sample_z_prior(self, mbsize):
        ...
        z = z.cuda(self.gpu_id) if self.gpu else z
        return z
    ...
    def forward(self, sentence, code):
        """
        Params:
        -------
        ...
        pad_words = pad_words.cuda(self.gpu_id) if self.gpu else pad_words
        ...
        # Decoder: sentence -> y
        y = self.forward_decoder(sentence, z, code)
    ...
    def sample_sentence(self, h, c, raw=False, temp=1):
        """
        ...
        word = word.cuda(self.gpu_id) if self.gpu else word
        word = Variable(word)  # '<start>'

        h = torch.cat([h, c], dim=2)
        ...
        for i in range(self.MAX_SENT_LEN):
            ...
            word = Variable(torch.LongTensor([idx]))
            word = word.cuda(self.gpu_id) if self.gpu else word
        ...
        if raw:
            ...
            return outputs.cuda(self.gpu_id) if self.gpu else outputs
        else:
            return outputs
    ...
    def controlSentence(self, sentence, t=0.5):
        self.eval()
        
        sentence = Variable(sentence).cuda(self.gpu_id)
        
        h = self.forward_encoder(sentence)
        c = self.sample_c_prior(1)
        ...
        sample_idxs = self.sample_sentence(h, c, temp=t)
        results.append(sample_idxs)
